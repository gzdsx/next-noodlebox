...
interface ModalDriverScheduleProps {
    onClose?: () => void;
    ids: string[] | React.Key[]
}

const ModalDriverSchedule = ({onClose, ids}: ModalDriverScheduleProps) => {
    ...
    const fetchSchedules = () => {
        setLoading(true);
        apiGet(`/deliveryer/shifts`, {weeks, deliveryers: ids.join(',')}).then(response => {
    ...
    const handleSubmit = () => {
        ...
        setLoading(true);
        apiPost(`/deliveryer/shifts`, {staffs: newShifts}).then(_ => {
    ...
    const handleCopyLastWeek = () => {
        setLoading(true);
        apiPost(`/deliveryer/shifts/copy-last-week`, {weeks}).then(() => {
    ...
    useEffect(() => {
        setTimeout(fetchSchedules);
    }, [weeks]);

    return (
        <Modal open={true} footer={null} width={'98%'} title={'司机排班'} onCancel={() => onClose?.()}>
            ...
            <div className={'mt-4 flex items-center justify-center'}>
                <Button type={'primary'} onClick={handleSubmit} loading={loading}>Save All</Button>
            </div>
        </Modal>
    );
};

export default ModalDriverSchedule;